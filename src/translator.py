        ...
        if isinstance(terms, list):
            ...
            match head:
                case "put_prim":  # (put_prim primitive) -> status 1
                    if not isinstance(terms[1], list):
                        ...
                        if terms[1].strip("\'\"") in variables_primitives.keys():
                            ...
                            asm.append({"opcode": "load", "addr_mod": "abs_addr", "addr": primitive,
                                        "term": "put_prim"})
                            asm.append({"opcode": "output", "addr_mod": "non_addr", "addr": 11,
                                        "term": "put_prim"})
                        else:
                            raise NotImplemented("Доделай пут чар через аски код или через символ")
                ...
                case "load_prim":  # (load_prim address) -> primitive
                    asm.append({"opcode": "load", "addr_mod": "abs_addr", "addr": int(terms[1]),
                                "term": "load_prim"})
                    asm.append({"opcode": "push", "addr_mod": "non_addr", "addr": None,
                                "term": "load_prim"})
                ...
                case "store_prim":  # (store_prim primitive address) -> status 1
                    if terms[2].strip("\'\"") in variables_primitives.keys():
                        ...
                        asm.append({"opcode": "load", "addr_mod": "abs_addr", "addr": address,
                                    "term": "store_prim", "comment": "push variable addr"})
                        asm.append({"opcode": "push", "addr_mod": "non_addr", "addr": None,
                                    "term": "store_prim", "comment": "push variable addr"})
                    ...
                    if terms[2].strip("\'\"") in variables_primitives.keys():
                        ...
                        asm.append({"opcode": "store", "addr_mod": "abs_addr", "addr": int(terms[2]),
                                    "term": "store_prim"})
                ...
                case "lisp_read":  # (lisp_read)
                    ...
                    asm.append(
                        {"opcode": "load", "addr_mod": "nep_addr", "addr": str_addr,
                         "term": "lisp_read", "comment": "return init memory for string"})
                ...
                case "var_input":
                    ...
                    asm.append({"opcode": "load", "addr_mod": "nep_addr", "addr": 1,
                                "term": "var_input", "comment": "return 1 if all ok"})
                    asm.append({"opcode": "push", "addr_mod": "non_addr", "addr": None,
                                "term": "var_input", "comment": "return 1 if all ok"})
                    return "$"
                ...
                case "var":
                    if isinstance(terms[2], str):
                        if terms[1].strip("\'\"") not in variables_primitives.keys():
                            if terms[2].strip("\'\"").isdigit() or terms[2].strip(
                                    "\'\"") in variables_primitives.keys():
                        ...
                        if terms[2].strip("\'\"") in variables_primitives.keys():
                            asm.append(
                                {"opcode": "load", "addr_mod": "abs_addr", "addr": variables_primitives[terms[2]],
                                 "term": "var", "comment": "Load concrete known value"})
                            asm.append(
                                {"opcode": "store", "addr_mod": "abs_addr", "addr": variables_primitives[terms[1]],
                                 "term": "var"})
                        else:
                        ...
                        if "read" not in terms[2]:
                            ...
                            if terms[2] == "$":
                                ...
                                asm.append({"opcode": "store", "addr_mod": "abs_addr",
                                            "addr": variables_primitives[terms[1]], "term": "var"})
                        else:
                            terms[2] = f(terms[2])
                    ...
                    if isinstance(terms[1], str) and isinstance(terms[2], str):
                        if terms[1].strip("\'\"") in variables_primitives.keys():
                            ...
                            asm.append({"opcode": "load", "addr_mod": "nep_addr", "addr": terms[1],
                                        "term": "+"})
                        if terms[2].strip("\'\"") in variables_primitives.keys():
                            asm.append({"opcode": "add", "addr_mod": "abs_addr", "addr": variables_primitives[terms[2]],
                                        "term": "+"})
                        else:
                        ...
                        if isinstance(terms[1], list) and isinstance(terms[2], str):
                            ...
                            if terms[2].strip("\'\"") in variables_primitives.keys():
                                asm.append(
                                    {"opcode": "add", "addr_mod": "abs_addr", "addr": variables_primitives[terms[2]],
                                     "term": "+"})
                            else:
                                asm.append(
                    ...
                    if isinstance(terms[1], str) and isinstance(terms[2], str):
                        if terms[1].strip("\'\"") in variables_primitives.keys():
                            ...
                            asm.append({"opcode": "load", "addr_mod": "nep_addr", "addr": terms[1],
                                        "term": "-"})
                        if terms[2].strip("\'\"") in variables_primitives.keys():
                            asm.append({"opcode": "sub", "addr_mod": "abs_addr", "addr": variables_primitives[terms[2]],
                                        "term": "-"})
                        else:
                        ...
                        if isinstance(terms[1], list) and isinstance(terms[2], str):
                            ...
                            if terms[2].strip("\'\"") in variables_primitives.keys():
                                asm.append(
                                    {"opcode": "sub", "addr_mod": "abs_addr", "addr": variables_primitives[terms[2]],
                                     "term": "-"})
                            else:
                                asm.append(
                    ...
                    if isinstance(terms[2], list) and isinstance(terms[3], str):
                        ...
                        if terms[2] == "$" and terms[3] != "$":
                            ...
                            if terms[3].strip("\'\"") in variables_primitives.keys():
                                asm.append(
                                    {"opcode": "load", "addr_mod": "abs_addr", "addr": variables_primitives[terms[3]],
                                     "term": "if"})
                            else:
                                asm.append(
